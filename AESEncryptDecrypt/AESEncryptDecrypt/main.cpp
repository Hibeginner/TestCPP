...
void TestDecrypt() {
    const std::string filePath = "appInit.js";
    FileReader tempFileReader(filePath);
    int fileSize = tempFileReader.GetFileSize();

    char* fileBuff = new char[fileSize];
    tempFileReader.ReadBuff(fileBuff, fileSize);
    char* outBuff = new char[fileSize];

    uint8_t key[] = { 0x41, 0x43, 0x54, 0x6a, 0x6a, 0x73, 0x67, 0x5f, 0x32, 0x30, 0x32, 0x32, 0x31, 0x32, 0x32, 0x32 };//ACTjjsg_20221222
    uint8_t iv[] = { 0x41, 0x43, 0x54, 0x6a, 0x6a, 0x73, 0x67, 0x5f, 0x32, 0x30, 0x32, 0x32, 0x31, 0x32, 0x32, 0x32 };
    struct AES_ctx ctx;
    AES_init_ctx_iv(&ctx, key, iv);
    AES_CBC_decrypt_buffer(&ctx, (unsigned char*)fileBuff, fileSize);//aes-128-cbc
    size_t actualFileSize = pkcs7_padding_data_length((unsigned char*)fileBuff, fileSize, 16);
